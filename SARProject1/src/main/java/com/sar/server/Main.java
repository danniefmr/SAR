...
public class Main {
   ...
   public Main() {
       ...
       try {
        ...
        // 3. Initialize EventBroadcaster FIRST (before Service)
        logger.info("Initializing Event Broadcaster...");
        this.eventBroadcaster = initializeEventBroadcaster();

        // 4. Initialize Service (which needs EventBroadcaster)
    ...
    public void startServer() {
        try {
            ...
            MainThread = new ServerThread(this, server, httpController, eventBroadcaster);
            MainSecureThread = new ServerThread(this, serverS, httpController, eventBroadcaster);
            MainThread.start();
